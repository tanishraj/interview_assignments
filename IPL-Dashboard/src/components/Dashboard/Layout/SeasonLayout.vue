  ...
  <div class="content">
    <div class="header">
      ...
      <div class="selected-year activeLink">
        <span :style="{ 'z-index': '4', position: 'relative' }">{{
          seasonDetail.year.toString()
        }}</span>
...
export default {
  ...
  data() {
    return {
      seasonDetail:
        this.$store.getters.particularSeason(this.$route.params.year) || "",
      date: "",
  ...
  computed: {
    style() {
      return `background: linear-gradient(45deg, #2b2b2b, ${this.seasonDetail.backgroundColor.slice(
        25,
        32
      )})`;
    },
