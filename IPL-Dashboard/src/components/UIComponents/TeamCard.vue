<template>
  <div :class="`team-card ${team.short}-card`">
    <div class="card">
      <div
        class="team-card-front"
        :style="
          `background: url(${checkImage(team.photo)}) center center no-repeat`
        "
      >
        <div class="content" :style="{ background: team.color }">
          <div class="logo">
            <img
              :src="checkImage(team.image)"
              :alt="team.name"
              :width="team.width"
              :style="{ marginTop: team.marginTop }"
            />
          </div>
          <div class="text">
          ...
          <div class="best-result">
            <img
              src="@/assets/img/misc/cup.png"
              alt
              style="height: 148px;z-index: 2;position: absolute; left: 0;"
            />
        ...
        <div class="content" :style="{ background: team.color }">
          ...
          <div class="performance">
            <div class="bar">
              <div
                class="total-matches"
                :style="{ color: team.color.slice(25, 34) }"
              >
                <span
                  class="number-of-matches"
                  :style="{ background: team.color.slice(25, 34) }"
                  >{{ this.totalMatches() }}</span
                >
                total Matches
              </div>
              <div
              ...
              <div class="tie" :style="{ color: team.color.slice(25, 34) }">
                <span
                  ...
                  :style="{ background: team.color.slice(25, 34) }"
                >
                  {{ this.team.tie.win + this.team.tie.lose }}
                  <sup style="font-size: 19px">*</sup>
                </span>
            ...
            <div class="info">
              <span style="{'background': team.color.slice(25, 34)}">*</span>T+W
              and T+L indicates matches tied and then won or lost by "Super
              Over"
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    checkImage(image) {
      const result = image.split("static");
      return require(`@/assets${result[1]}`);
    },
    style(photo) {
